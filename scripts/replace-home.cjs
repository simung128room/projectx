...
const categoryStart = content.indexOf('{/* ===== Latest Products Label ===== */}');

if (heroStart !== -1 && categoryStart !== -1) {
  content = content.substring(0, heroStart) + heroSectionNew + '\n\n      ' + content.substring(categoryStart);
  fs.writeFileSync(file, content);
  console.log('Replaced Hero Section in HomeView');
} else {
  console.log('Could not find hero or category bounds');
}
