...
หากพบปัญหาในการใช้งาน กรุณาติดต่อผู้ดูแลระบบผ่านช่องทางติดต่อบนเว็บไซต์`,
  ...
  const getViewPath = useCallback((view: ViewType | string) => {
    ...
    return pathByView[view] ?? String(view);
  }, []);

  const resolvePathToView = useCallback((rawPath: string): ViewType => {
    ...
    let path = decodedPath.replace(/^\/+|\/+$/g, "").toLowerCase();
    if (path === "") {
      const hostname = window.location.hostname;
    ...
    const routeAliases: Record<string, ViewType> = {
      register: "signup",
    ...
    return routeAliases[path] ?? (path as ViewType);
  }, []);

  const setActiveView = useCallback(
    (view: any) => {
      if (activeView === view) return;
      const pathSuffix = getViewPath(view);
      window.history.pushState(null, "", pathSuffix ? `/${pathSuffix}` : "/");
      setRawActiveView(view);
    },
    [activeView, getViewPath],
  );

  // Handle URL pathname routing
  useEffect(() => {
    const handlePopState = () => {
      const path = resolvePathToView(window.location.pathname);
