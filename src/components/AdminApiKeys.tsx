      ...
      <div className="flex justify-between items-center bg-zinc-950/20 p-4 border border-zinc-900 rounded-lg">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            (API Keys Management)</h2>
          <p className="text-xs text-zinc-500 mt-1">
            Token  API /api/check</p>
        </div>
        <button
        ...
        <form onSubmit={handleAddKey} className="bg-[#0B0C0E] border border-zinc-800 rounded-lg overflow-hidden shrink-0 shadow-lg p-6 space-y-5 animate-in fade-in zoom-in-95 duration-200">
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              placeholder=" Python Checker Script "
          ...
          <div className="flex items-center gap-2.5 py-1">
            ...
            />
            <label htmlFor="isLifetime" className="text-sm font-bold text-zinc-200 cursor-pointer select-none">(Lifetime Period)</label>
          </div>

          {!isLifetime && (
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-zinc-400 uppercase">()</label>
              <input
                type="number"
                min="1"
                required
          ...
          <table className="w-full text-left border-collapse text-sm text-zinc-400">
            <thead>
              <tr className="bg-[#0B0C0E] border-b border-zinc-800 text-zinc-550 uppercase text-xs tracking-wider">
                ...
                <th className="px-5 py-3.5 font-bold">API Key</th>
                <th className="px-5 py-3.5 font-bold">(Token)</th>
                <th className="px-5 py-3.5 font-bold"></th>
                <th className="px-5 py-3.5 font-bold"></th>
                <th className="px-5 py-3.5 font-bold"></th>
                <th className="px-5 py-3.5 font-bold text-right"></th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="text-center py-12">
                     ...
                     <span className="text-xs text-zinc-550 font-semibold">...</span>
                  </td>
                </tr>
              ) : apiKeys.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-zinc-600 font-semibold text-xs"></td>
                </tr>
              ) : (
                apiKeys.map(k => (
                  <tr key={k.key} className="hover:bg-zinc-900/10 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-xs text-zinc-500 whitespace-nowrap">
                      {k.expires_at ? new Date(k.expires_at).toLocaleString('th-TH') : <span className="font-bold text-zinc-400"> Lifetime</span>}
                    </td>
                    <td className="px-5 py-3.5 text-xs text-zinc-500 whitespace-nowrap">
                      {k.last_used ? new Date(k.last_used).toLocaleString('th-TH') : <span className="italic text-zinc-650"></span>}
                    </td>
