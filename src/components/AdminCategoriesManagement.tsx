    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="bg-purple-600 hover:bg-[#1D4ED8] text-gray-900 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2"
        >
      ...
      {(isAdding || editingCategory) && (
        <div className="bg-[#0B0D0F] border rounded-xl overflow-hidden mb-8 shadow-lg transition-all">
          <div className="bg-gray-100/50 p-6 sm:p-8 flex items-center justify-between border-b border-gray-200">
            <div>
            ...
            <div className="p-3 bg-[#0B0D0F] shadow-sm border border-gray-200 rounded-2xl">
          ...
          <div className="p-6 sm:p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4 md:col-span-1">
                <div className="group">
                  <label className="block text-sm font-bold text-gray-600 mb-2 group-focus-within:text-[#2563EB] transition-colors">
                  ...
                  <input 
                    ...
                    className="w-full bg-gray-100 focus:bg-[#0B0D0F] border border-gray-200 focus:border-[#3B82F6]/40 focus:ring-4 focus:ring-[#3B82F6]/30 rounded-2xl px-4 py-3 text-sm font-medium transition-all" 
                    placeholder="เช่น game_accounts (อักษรภาษาอังกฤษ)" 
                  />
                  <p className="text-xs text-gray-600 mt-2 ml-1">สำหรับใช้ในระบบ โปรดใช้ภาษาอังกฤษ</p>
                </div>
                ...
                <div className="group">
                  ...
                  />
                  <p className="text-xs text-gray-600 mt-2 ml-1">ชื่อหมวดหมู่ที่จะแสดงให้ผู้ใช้งานเห็น</p>
                </div>
              </div>
              ...
              <div className="space-y-4 md:col-span-1 flex flex-col">
                <div className="group flex-1">
                  ...
                  <textarea 
                    ...
                    className="w-full h-[124px] bg-gray-100 focus:bg-[#0B0D0F] border border-gray-200 focus:border-[#3B82F6]/40 focus:ring-4 focus:ring-[#3B82F6]/30 rounded-2xl px-4 py-3 text-sm font-medium transition-all resize-none" 
              ...
              <div className="md:col-span-2 pt-4 border-t border-gray-200">
                <label className="block text-sm font-bold text-gray-600 mb-3">รูปภาพหน้าปกหมวดหมู่</label>
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1 space-y-4">
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <ImageIcon className="h-5 w-5 text-gray-600 group-focus-within:text-[#2563EB] transition-colors" />
                      </div>
                      <input 
                        type="text" 
                        value={formData.bannerUrl || ''} 
                        onChange={e => setFormData({...formData, bannerUrl: e.target.value})} 
                        className="w-full bg-gray-100 focus:bg-[#0B0D0F] border border-gray-200 focus:border-[#3B82F6]/40 focus:ring-4 focus:ring-[#3B82F6]/30 rounded-2xl pl-11 pr-4 py-3 text-sm font-medium transition-all" 
                  ...
                  {formData.bannerUrl ? (
                    <div className="w-full md:w-64 h-32 rounded-2xl overflow-hidden border border-gray-200 shadow-sm relative group bg-gray-100 shrink-0">
                      ...
                      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <p className="text-gray-900 text-xs font-bold text-center px-4 drop-shadow-md">พรีวิวรูปภาพหน้าปก</p>
                      </div>
                    </div>
                  ) : (
                    <div className="w-full md:w-64 h-32 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-600 shrink-0 bg-gray-100/50">
            ...
            <div className="flex gap-3 justify-end pt-6 mb-2 border-t border-gray-200">
              <button onClick={() => { setIsAdding(false); setEditingCategory(null); }} className="px-6 py-3 rounded-2xl font-bold bg-[#0B0D0F] border border-gray-200 text-gray-600 hover:bg-gray-100 hover:text-gray-900 transition-colors shadow-sm">
                ยกเลิก
              </button>
              <button onClick={saveCategory} className="px-6 py-3 rounded-2xl font-bold bg-purple-600 text-gray-900 hover:bg-[#1D4ED8] hover:shadow-lg hover:shadow-lg/20 flex items-center gap-2 transition-all active:scale-95">
      ...
      <div className="bg-[#0B0D0F] border text-sm border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-100 border-b border-gray-200 text-gray-500 uppercase text-xs tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-zinc-100">
              {categories.map((c) => (
                <tr key={c.id} className="hover:bg-gray-100/80 transition-colors group">
                  <td className="px-6 py-4">
                    {c.bannerUrl ? (
                      <div className="w-24 h-14 rounded-xl overflow-hidden relative shadow-sm border border-gray-200 group-hover:border-[#3B82F6]/30 transition-colors">
                      ...
                      <div className="w-24 h-14 bg-gray-100 rounded-xl flex flex-col items-center justify-center text-gray-600 border border-gray-200 border-dashed">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="font-black text-gray-900 text-base">{c.title}</span>
                      <div className="flex items-center gap-1.5 mt-1">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-gray-100 text-gray-500 font-mono">
                          {c.name}
                        </span>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <p className="text-gray-500 text-sm max-w-[250px] truncate" title={c.subtitle}>
                      {c.subtitle || <span className="text-gray-700 italic">ไม่ได้ระบุรายละเอียด</span>}
                    </p>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2 opacity-100 md:opacity-50 md:group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setManagingProductsForCategory(c)} className="p-2 border border-purple-500/30 bg-blue-600/10 text-blue-600 hover:bg-blue-600 hover:text-gray-900 rounded-xl transition-colors shadow-sm flex items-center gap-2 px-3" title="เพิ่ม/จัดการสินค้าในหมวดหมู่นี้">
                        <ShoppingCart className="w-4 h-4" />
                        <span className="text-xs font-bold hidden sm:inline">จัดการสินค้า</span>
                      </button>
                      <button onClick={() => { setEditingCategory(c); setFormData(c); setIsAdding(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="p-2 border border-[#3B82F6]/30 bg-purple-600/10 text-gray-900 hover:bg-purple-600 hover:text-gray-900 rounded-xl transition-colors shadow-sm" title="แก้ไข">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => deleteCategory(c.id)} className="p-2 border border-red-500/30 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-gray-900 rounded-xl transition-colors shadow-sm" title="ลบ">
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-600">
                      ...
                      <p className="font-bold text-gray-500">ยังไม่มีข้อมูลหมวดหมู่สินค้าในระบบ</p>
      ...
      {managingProductsForCategory && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-end p-0 z-50">
          <div className="bg-[#0B0D0F] border-l border-gray-200 w-full max-w-2xl h-full shadow-2xl relative p-6 sm:p-8 flex flex-col overflow-y-auto animate-in slide-in-from-right-full duration-300">
            <button 
              onClick={() => setManagingProductsForCategory(null)}
              className="absolute top-4 right-4 p-2 text-gray-500 hover:text-gray-900 bg-gray-50 border border-gray-200 rounded-full hover:bg-gray-200 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            <h2 className="text-xl font-bold text-gray-900 mb-2 flex items-center gap-2 shrink-0">
              <ShoppingCart className="w-5 h-5 text-blue-600" />
              จัดการสินค้าในหมวดหมู่
            </h2>
            <p className="text-sm font-medium text-gray-500 mb-6 shrink-0">
                  ...
                  return (
                    <div 
                      ...
                      className={`flex items-center gap-4 p-3 rounded-2xl border cursor-pointer transition-colors ${
                        isChecked ? 'bg-blue-600/10 border-purple-500/30' : 'bg-gray-50/50 border-gray-200 hover:bg-gray-50 hover:border-gray-200'
                      }`}
                    >
                      <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 border ${
                        isChecked ? 'bg-purple-600 border-purple-500 text-gray-900' : 'bg-gray-50 border-gray-200'
                      }`}>
            ...
            <div className="flex gap-3 justify-end pt-4 border-t border-gray-200 shrink-0">
              <button 
                onClick={() => setManagingProductsForCategory(null)} 
                className="px-6 py-3 rounded-xl font-bold bg-gray-50 border border-gray-200 text-gray-600 hover:text-gray-900 transition-colors"
              >
                ยกเลิก
              </button>
              <button 
                ...
                className="px-6 py-3 rounded-xl font-bold bg-purple-600 text-gray-900 hover:bg-blue-600 flex items-center gap-2 transition-colors disabled:opacity-50"
              >
