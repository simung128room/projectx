...
export const AdminCategoriesManagement: React.FC<AdminCategoriesManagementProps> = ({ categories, setCategories, products = [], setProducts }) => {
  ...
  const saveCategoryProducts = async () => {
    ...
    try {
      ...
      Swal.fire({ title: 'สำเร็จ', text: 'อัปเดตสินค้าในหมวดหมู่เรียบร้อย', icon: 'success', background: '#121212', color: '#fff', confirmButtonColor: '#364153' });
      setManagingProductsForCategory(null);
    } catch (err) {
      console.error(err);
      Swal.fire({ title: 'ข้อผิดพลาด', text: 'ไม่สามารถอัปเดตข้อมูลสินค้าได้', icon: 'error', background: '#121212', color: '#fff', confirmButtonColor: '#EF4444' });
    } finally {
  ...
  const saveCategory = async () => {
    if (!formData.name || !formData.title) {
      Swal.fire({ title: 'ข้อผิดพลาด', text: 'กรุณากรอกชื่ออ้างอิงและหัวข้อหลัก', icon: 'error', background: '#121212', color: '#fff', confirmButtonColor: '#EF4444' });
      return;
    }
    ...
    try {
      ...
      Swal.fire({ title: 'สำเร็จ', text: 'บันทึกหมวดหมู่เรียบร้อย', icon: 'success', background: '#121212', color: '#fff', confirmButtonColor: '#364153' });
    } catch (err) {
      console.error(err);
      Swal.fire({ title: 'Error', text: 'ไม่สามารถบันทึกได้', icon: 'error', background: '#121212', color: '#fff', confirmButtonColor: '#EF4444' });
    }
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire({ title: 'สำเร็จ', text: 'ลบหมวดหมู่เรียบร้อย', icon: 'success', background: '#121212', color: '#fff', timer: 1000, showConfirmButton: false });
        } catch (err) {
          Swal.fire({ title: 'Error', text: 'ไม่สามารถลบหมวดหมู่ได้', icon: 'error', background: '#121212', color: '#fff' });
        }
      }
    });
  };
      ...
      {(isAdding || editingCategory) && (
        <div className="bg-[#0B0C0E] border border-[#374151] rounded-md overflow-hidden mb-8 transition-all duration-200 shadow-md">
          <div className="bg-zinc-950 p-6 flex items-center justify-between border-b border-zinc-850">
            ...
            <div className="p-2.5 bg-[#050505]/50 border border-[#374151] rounded-md">
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-5 md:col-span-1">
                <div className="group">
                  ...
                  <input 
                    ...
                    className="w-full bg-zinc-950 border border-[#374151] focus:border-[#364153]/60 rounded-md px-4 py-2.5 text-white text-sm focus:outline-none transition-colors" 
              ...
              <div className="space-y-5 md:col-span-1 flex flex-col">
                <div className="group flex-1">
                  ...
                  <textarea 
                    ...
                    className="w-full h-[122px] bg-zinc-950 border border-[#374151] focus:border-[#364153]/60 rounded-md px-4 py-2.5 text-white text-sm focus:outline-none transition-colors resize-none" 
              ...
              <div className="md:col-span-2 pt-4 border-t border-zinc-850">
                <label className="block text-xs font-medium text-zinc-400 mb-2">รูปภาพหน้าปกหมวดหมู่ (URL)</label>
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        className="w-full bg-zinc-950 border border-[#374151] focus:border-[#364153]/60 rounded-md pl-10 pr-4 py-2.5 text-white text-sm focus:outline-none transition-colors" 
                  ...
                  {formData.bannerUrl ? (
                    <div className="w-full md:w-56 h-28 overflow-hidden rounded-md border border-[#374151] bg-zinc-950 shrink-0 relative group">
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <p className="text-white text-[10px] font-medium">พรีวิวรูปภาพหน้าปก</p>
                      </div>
                    </div>
                  ) : (
            ...
            <div className="flex gap-2 justify-end pt-5 border-t border-zinc-850">
              <button 
                ...
                className="border border-[#374151] bg-[#050505]/60 hover:bg-[#0a0a0a] text-zinc-400 hover:text-white px-4 py-2 text-xs font-medium rounded-md transition-colors"
              >
                ยกเลิก
              </button>
              <button 
                onClick={saveCategory} 
                className="bg-[#364153] hover:bg-[#364153] text-white px-4 py-2 text-xs font-medium flex items-center gap-2 rounded-md transition-colors active:scale-95"
              >
      ...
      <div className="bg-[#121212] border border-[#374151] rounded-md overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-sm text-zinc-400">
            <thead className="bg-[#0B0C0E] border-b border-[#374151] text-zinc-500 uppercase text-xs tracking-wider">
              <tr>
                ...
                <tr key={c.id} className="hover:bg-[#050505]/10 transition-all group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      <div className="mt-1">
                        <span className="inline-flex items-center px-1.5 py-0.5 text-[9px] font-medium bg-zinc-950 text-zinc-500 font-mono border border-[#374151]/80 rounded">
                          {c.name}
                        </span>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-end p-0 z-50">
          <div className="bg-zinc-950 border-l border-[#374151] w-full max-w-xl h-full relative p-6 sm:p-8 flex flex-col justify-between overflow-hidden shadow-md">
            <button 
              ...
              className="absolute top-5 right-5 p-2 text-zinc-400 hover:text-white bg-[#050505] border border-[#374151] hover:bg-zinc-850 rounded-md transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
                      ...
                      <div 
                        ...
                        className={`flex items-center gap-3.5 p-3 rounded-md border cursor-pointer transition-all ${ 
                          isChecked 
                            ...
                            : 'bg-[#050505]/30 border-[#374151] hover:bg-[#050505]/60 hover:border-[#374151]' 
                        }`}
                      >
            ...
            <div className="flex gap-2 justify-end pt-4 border-t border-[#374151] shrink-0">
              <button 
                onClick={() => setManagingProductsForCategory(null)} 
                className="px-4 py-2 text-xs font-medium border border-[#374151] bg-[#050505]/60 hover:bg-[#0a0a0a] text-zinc-400 hover:text-white rounded-md transition-colors"
              >
                ยกเลิก
              </button>
              <button 
                ...
                className="px-5 py-2 text-xs font-medium bg-[#364153] hover:bg-[#364153] text-white flex items-center gap-2 rounded-md transition-colors disabled:opacity-50"
              >
