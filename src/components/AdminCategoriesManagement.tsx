    ...
    <div className="space-y-6">
      ...
      {(isAdding || editingCategory) && (
        <div className="bg-[#0B0D0F] border rounded-xl overflow-hidden mb-8 shadow-lg transition-all">
          <div className="bg-[#121212]/50 p-6 sm:p-8 flex items-center justify-between border-b border-white/10">
            <div>
            ...
            <div className="p-3 bg-[#0B0D0F] shadow-sm border border-white/10 rounded-2xl">
          ...
          <div className="p-6 sm:p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4 md:col-span-1">
                <div className="group">
                  <label className="block text-sm font-bold text-zinc-400 mb-2 group-focus-within:text-[#2563EB] transition-colors">
                  ...
                  <input 
                    ...
                    className="w-full bg-[#121212] focus:bg-[#0B0D0F] border border-white/10 focus:border-[#3B82F6]/40 focus:ring-4 focus:ring-[#3B82F6]/30 rounded-2xl px-4 py-3 text-sm font-medium transition-all" 
              ...
              <div className="space-y-4 md:col-span-1 flex flex-col">
                <div className="group flex-1">
                  ...
                  <textarea 
                    ...
                    className="w-full h-[124px] bg-[#121212] focus:bg-[#0B0D0F] border border-white/10 focus:border-[#3B82F6]/40 focus:ring-4 focus:ring-[#3B82F6]/30 rounded-2xl px-4 py-3 text-sm font-medium transition-all resize-none" 
              ...
              <div className="md:col-span-2 pt-4 border-t border-white/10">
                <label className="block text-sm font-bold text-zinc-400 mb-3">รูปภาพหน้าปกหมวดหมู่</label>
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1 space-y-4">
                    <div className="relative group">
                      ...
                      <input 
                        ...
                        className="w-full bg-[#121212] focus:bg-[#0B0D0F] border border-white/10 focus:border-[#3B82F6]/40 focus:ring-4 focus:ring-[#3B82F6]/30 rounded-2xl pl-11 pr-4 py-3 text-sm font-medium transition-all" 
                  ...
                  {formData.bannerUrl ? (
                    <div className="w-full md:w-64 h-32 rounded-2xl overflow-hidden border border-white/10 shadow-sm relative group bg-[#121212] shrink-0">
                    ...
                    <div className="w-full md:w-64 h-32 rounded-2xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center text-zinc-400 shrink-0 bg-[#121212]/50">
            ...
            <div className="flex gap-3 justify-end pt-6 mb-2 border-t border-white/10">
              <button onClick={() => { setIsAdding(false); setEditingCategory(null); }} className="px-6 py-3 rounded-2xl font-bold bg-[#0B0D0F] border border-white/10 text-zinc-400 hover:bg-[#121212] hover:text-white transition-colors shadow-sm">
                ยกเลิก
              </button>
      ...
      <div className="bg-[#0B0D0F] border text-sm border-white/10 rounded-xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#121212] border-b border-white/10 text-zinc-500 uppercase text-xs tracking-wider">
              <tr>
                ...
                <tr key={c.id} className="hover:bg-[#121212]/80 transition-colors group">
                  <td className="px-6 py-4">
                    {c.bannerUrl ? (
                      ...
                      <div className="w-24 h-14 bg-[#121212] rounded-xl flex flex-col items-center justify-center text-zinc-400 border border-white/10 border-dashed">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold bg-[#121212] text-zinc-500 font-mono">
                          {c.name}
                        </span>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2 opacity-100 md:opacity-50 md:group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setManagingProductsForCategory(c)} className="p-2 border border-purple-500/30 bg-blue-600/10 text-blue-600 hover:bg-blue-600 hover:text-white rounded-xl transition-colors shadow-sm flex items-center gap-2 px-3" title="เพิ่ม/จัดการสินค้าในหมวดหมู่นี้">
      ...
      {managingProductsForCategory && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-end p-0 z-50">
          <div className="bg-[#0B0D0F] border-l border-white/10 w-full max-w-2xl h-full shadow-2xl relative p-6 sm:p-8 flex flex-col overflow-y-auto animate-in slide-in-from-right-full duration-300">
            <button 
              onClick={() => setManagingProductsForCategory(null)}
              className="absolute top-4 right-4 p-2 text-zinc-500 hover:text-white bg-[#0a0a0a] border border-white/10 rounded-full hover:bg-[#1e1e1e] transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
                  ...
                  return (
                    <div 
                      ...
                      className={`flex items-center gap-4 p-3 rounded-2xl border cursor-pointer transition-colors ${
                        isChecked ? 'bg-blue-600/10 border-purple-500/30' : 'bg-[#0a0a0a]/50 border-white/10 hover:bg-[#0a0a0a] hover:border-white/10'
                      }`}
                    >
                      <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 border ${
                        isChecked ? 'bg-purple-600 border-purple-500 text-white' : 'bg-[#0a0a0a] border-white/10'
                      }`}>
                        {isChecked && <Check className="w-4 h-4" />}
                      </div>
                      <div className="w-12 h-12 bg-[#121212] rounded-xl overflow-hidden shrink-0">
                         {p.imageUrl ? (
            ...
            <div className="flex gap-3 justify-end pt-4 border-t border-white/10 shrink-0">
              <button 
                onClick={() => setManagingProductsForCategory(null)} 
                className="px-6 py-3 rounded-xl font-bold bg-[#0a0a0a] border border-white/10 text-zinc-400 hover:text-white transition-colors"
              >
                ยกเลิก
              </button>
              <button 
                ...
                className="px-6 py-3 rounded-xl font-bold bg-purple-600 text-white hover:bg-blue-600 flex items-center gap-2 transition-colors disabled:opacity-50"
              >
