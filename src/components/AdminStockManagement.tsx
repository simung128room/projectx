      ...
      <div className="animate-in fade-in zoom-in-95 duration-200">
         <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-4">
                <button onClick={() => setSelectedProduct(null)} className="p-2 bg-[#0B0D0F] border border-white/10 hover:bg-[#121212] rounded-xl transition-colors">
            ...
            <div className="flex-1 max-w-sm relative">
                ...
                <input 
                  ...
                  className="w-full bg-[#121212] border border-white/10 rounded-xl py-2 pl-9 pr-4 text-sm text-white focus:border-[#2563EB] focus:outline-none transition-colors"
                />
            </div>
         </div>
         
         <div className="bg-[#0B0D0F] border border-white/10 rounded-xl p-4 overflow-hidden">
             {loading ? (
                ...
                <div className="space-y-2">
                    {paginated.map(({item, originalIndex}, idx) => (
                        <div key={idx} className="flex bg-[#121212] p-4 rounded-xl border border-white/10 items-center justify-between hover:border-white/10 transition-colors group">
                            ...
                            <div className="flex gap-2 opacity-100 sm:opacity-50 sm:group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEdit(originalIndex, item)} className="p-2 bg-[#0a0a0a] border border-white/10 hover:border-blue-500/50 hover:bg-white/100/10 rounded-lg text-zinc-400 hover:text-blue-400 transition-all">
                                    <Edit2 className="w-4 h-4"/>
                                </button>
                                <button onClick={() => handleDelete(originalIndex)} className="p-2 bg-[#0a0a0a] border border-white/10 hover:border-red-500/50 hover:bg-red-500/10 rounded-lg text-zinc-400 hover:text-red-400 transition-all">
         ...
         {totalPages > 1 && (
            <div className="flex items-center gap-4 justify-between bg-[#0B0D0F] border border-white/10 rounded-2xl p-4 mt-4">
               <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-5 py-2 bg-[#0a0a0a] border border-white/10 hover:bg-[#1e1e1e] rounded-xl text-white text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed transition-colors">ก่อนหน้า</button>
               <span className="text-zinc-400 text-sm font-bold">หน้า {page} จาก {totalPages}</span>
               <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-5 py-2 bg-[#0a0a0a] border border-white/10 hover:bg-[#1e1e1e] rounded-xl text-white text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed transition-colors">ถัดไป</button>
            </div>
         )}
      </div>
    );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      {productsByCategory.length === 0 ? (
          <div className="bg-[#0B0D0F] border border-white/10 rounded-xl p-12 text-center">
          ...
          <div className="space-y-6">
              {productsByCategory.map((cat: any) => (
                 <div key={cat.id} className="bg-[#0B0D0F] border border-white/10 rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                       {cat.products.map((p: any) => {
                           ...
                           return (
                               <div key={p.id} onClick={() => setSelectedProduct(p)} className="p-4 bg-[#121212] border border-white/10 hover:border-[#3B82F6]/30 rounded-2xl cursor-pointer hover:bg-purple-600/5 transition-all flex items-center justify-between group">
                                  <div>
                                  ...
                                  <div className="w-8 h-8 rounded-full bg-[#0a0a0a] border border-white/10 flex items-center justify-center opacity-50 group-hover:opacity-100 group-hover:bg-purple-600 group-hover:border-[#3B82F6] transition-all">
