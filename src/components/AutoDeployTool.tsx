...
export const AutoDeployTool: React.FC<AutoDeployToolProps> = ({ onBack }) => {
  ...
  const validateSubdomain = (val: string) => {
    if (!val) return '';
    if (!/^[a-z0-9-]+$/.test(val)) return ' (a-z),  (0-9)  (-)';
    if (val.length< 3) return '3';
    if (val.length > 20) return ' 20 ';
    return '';
  };
  ...
  return (<div className="space-y-6 w-full animate-in fade-in zoom-in duration-300 max-w-4xl mx-auto">
      {/* Header View Row */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-white flex items-center gap-3">
            <Globe className="w-8 h-8 text-emerald-500" />
            (Auto Deploy Subdomain)</h2>
          <p className="text-muted-foreground mt-2 text-sm leading-relaxed">
            DirectAdmin API   MySQL  XWorms</p>
        </div>
        {onBack && (
          <button 
            onClick={onBack}
            ...
            {status === 'idle' && (
              ...
              >
                ...
                <div className="space-y-3">
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider"></label>
                  <div className="flex bg-black/40 border border-border border-2 focus-within:border-emerald-500/50 rounded-xl overflow-hidden transition-all duration-300">
                    <input 
                      type="text"
                      placeholder=": store99, mydeal, hyper"
                      value={subdomain}
            ...
            {status === 'loading' && (
              ...
              >
                <div className="text-center space-y-3 pb-2">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-white">DirectAdmin Deployer</h3>
                    <p className="text-zinc-500 text-xs font-semibold">API  10–20</p>
                  </div>
                </div>
                ...
                <div className="bg-black/30 border border-white/5 rounded-2xl p-4.5 space-y-3.5 text-xs font-medium font-sans">
                  <div className={`flex items-center gap-3 ${currentStep >= 1 ? 'text-white' : 'text-zinc-600'}`}>
                    ...
                    <span className={currentStep > 1 ? 'line-through text-zinc-500' : ''}> DirectAdmin Panel</span>
                  </div>
                  ...
                  <div className={`flex items-center gap-3 ${currentStep >= 2 ? 'text-white' : 'text-zinc-600'}`}>
                    ...
                    <span className={currentStep > 2 ? 'line-through text-zinc-500' : ''}> xworms  public_html</span>
                  </div>
                  ...
                  <div className={`flex items-center gap-3 ${currentStep >= 3 ? 'text-white' : 'text-zinc-600'}`}>
                    ...
                    <span className={currentStep > 3 ? 'line-through text-zinc-500' : ''}> SQL</span>
                  </div>
                  ...
                  <div className={`flex items-center gap-3 ${currentStep >= 4 ? 'text-white' : 'text-zinc-600'}`}>
                    ...
                    <span className={currentStep > 4 ? 'line-through text-zinc-500' : ''}></span>
                  </div>
            ...
            {status === 'error' && (
              ...
              >
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-lg font-black text-white"></h3>
                  <p className="text-red-400/80 text-[11px] leading-relaxed max-w-sm mx-auto">{errorMessage || ''}</p>
                </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-[#09090d] border border-white/5 rounded-3xl p-6 space-y-4">
            <h4 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2 border-b border-white/5 pb-2.5">
              <Server className="w-4 h-4 text-[#a78bfa]" /> </h4>
            
            <ul className="space-y-3 text-xs leading-relaxed text-zinc-400">
              <li className="flex gap-2.5 items-start">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 shrink-0" />
                <div>
                  <p className="font-bold text-zinc-200">Bought</p>
              ...
              <li className="flex gap-2.5 items-start">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 shrink-0" />
                <div>
                  <p className="font-bold text-zinc-200">Truemoney Wallet</p>
                  <p className="text-[10px] mt-0.5"></p>
                </div>
              </li>
              <li className="flex gap-2.5 items-start">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 shrink-0" />
                <div>
                  <p className="font-bold text-zinc-200">(admin/admin)</p>
                  <p className="text-[10px] mt-0.5"></p>
                </div>
              </li>
            </ul>
          ...
          <div className="bg-purple-950/10 border border-purple-900/10 rounded-3xl p-6 flex gap-3.5 items-start">
            ...
            <div className="space-y-1">
              <h4 className="text-xs font-black text-white uppercase tracking-wider">DirectAdmin</h4>
