...
export const CategoryProductsView: React.FC<CategoryProductsViewProps> = ({ 
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pt-24 font-sans text-zinc-900">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        <div className="flex items-center gap-4">
          <button 
            onClick={onBack}
            className="p-3 bg-white hover:bg-zinc-100 border border-zinc-200 rounded-full transition-colors group shadow-sm shrink-0"
          >
          ...
          <div>
            ...
            <p className="text-sm font-medium text-zinc-500 mt-3 flex items-center gap-2">
              <Star className="w-4 h-4 text-amber-500" /> {categoryInfo?.subtitle || `พบสินค้าทั้งหมด ${filteredProducts.length} รายการ`}
            </p>
          </div>
        </div>
      </div>
