...
export const CategoryProductsView: React.FC<CategoryProductsViewProps> = ({
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 font-sans text-foreground bg-transparent">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="p-3 bg-card hover:bg-secondary border border-[#1f293d] rounded-xl transition-colors group shrink-0 cursor-pointer outline-none"
          >
            ...
            <h1 className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight flex items-center gap-3">
              {category === "all" ? (
                <div className="flex items-center gap-3 uppercase">
                  <div className="w-12 h-12 bg-card border border-[#1f293d] text-primary flex items-center justify-center rounded-xl">
      ...
      {category !== "all" && categoryInfo?.bannerUrl && (
        <div 
          className="relative w-full rounded-2xl overflow-hidden border border-[#1f293d] bg-card mb-10 group shadow-sm"
      ...
      {!filteredProducts || filteredProducts.length === 0 ? (
        <div className="border border-dashed border-[#1f293d] bg-card rounded-2xl p-16 text-center">
          ...
          {visibleProducts.length < filteredProducts.length && (
            <div className="mt-12 flex justify-center">
              <button
                ...
                className="px-8 py-3 bg-card border border-[#1f293d] text-foreground hover:bg-secondary rounded-xl font-bold transition-all active:scale-95 cursor-pointer outline-none"
              >
