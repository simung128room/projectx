  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 md:p-8 animate-in fade-in duration-500 font-sans pb-20">
      <div className="flex items-center gap-4 mb-8">
        <button 
          onClick={onBack}
          className="w-10 h-10 bg-[#0B0D0F] border border-gray-200 rounded-full flex items-center justify-center text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-all shadow-sm"
        >
        ...
        <div>
          ...
          <p className="text-sm font-medium text-gray-500 mt-1">Checker Logs / รายการตรวจสอบไอดีล่าสุดของคุณ</p>
        </div>
      </div>

      <div className="bg-[#0B0D0F] border border-gray-200 rounded-xl p-6 shadow-sm overflow-hidden flex flex-col h-[70vh]">
        <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-gray-500 mt-0.5">บันทึกการทำงานล่าสุด (100 รายการ)</p>
            </div>
          </div>
        </div>

        <div className="flex-1 bg-gray-100 border border-gray-200 p-6 rounded-2xl text-[13px] font-mono overflow-auto scrollbar-thin scrollbar-thumb-zinc-300">
          {logs.length === 0 && (
            <div className="text-gray-600 flex flex-col items-center justify-center h-full gap-4 opacity-70">
          ...
          <div className="flex flex-col-reverse">
            {[...logs].map(log => {
              let lightModeColor = 'text-gray-600';
              ...
              return (
                <div key={log.id} className={`${lightModeColor} mb-2 flex items-start gap-3 break-all whitespace-pre-wrap leading-relaxed`}>
                  <span className="shrink-0 text-gray-600 font-medium">[{log.time}]</span>
