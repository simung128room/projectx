...
export const ContactView: React.FC<ContactViewProps> = ({ 
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4 md:p-8 animate-in fade-in duration-500 font-sans text-white">
      <button 
        onClick={onBack}
        className="mb-6 flex items-center gap-2 text-zinc-400 hover:text-white hover:shadow-sm text-sm font-medium transition-all bg-[#11131a] hover:bg-[#161a26] px-4 py-2 border border-[#1f293d] rounded-full w-fit cursor-pointer shadow-sm outline-none"
      >
        <ArrowLeft className="w-4 h-4" /> ย้อนกลับ
      </button>

      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8 bg-[#11131a] border border-[#1f293d] p-6 rounded-2xl shadow-[0_4px_20px_-8px_rgba(0,0,0,0.5)]">
        <div className="flex items-center gap-4">
          <div className="bg-[#161a26] border border-[#2d3748] text-blue-500 p-4 rounded-xl">
            <MessageSquare className="w-8 h-8" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">ติดต่อเรา / ขอความช่วยเหลือ</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <a 
          ...
          className="bg-[#11131a] border border-[#1f293d] hover:border-[#5865F2] hover:shadow-[0_4px_20px_-8px_rgba(88,101,242,0.3)] transition-all p-8 flex flex-col items-center justify-center text-center group cursor-pointer relative overflow-hidden rounded-2xl shadow-sm outline-none"
          onClick={(e) => {
            if (discordLink === '#') {
              e.preventDefault();
            }
        ...
        >
          <div className="w-20 h-20 bg-[#161a26] border border-[#2d3748] flex items-center justify-center mb-6 rounded-2xl group-hover:scale-110 transition-transform">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Discord Official</h2>
          <p className="text-zinc-500 text-sm font-medium">พูดคุย สอบถามปัญหา และติดตามข่าวสารล่าสุดผ่านทางดิสคอร์ด</p>
          <div className="mt-6 px-6 py-2.5 bg-[#161a26] hover:bg-[#4752C4] border border-[#2d3748] group-hover:text-white group-hover:border-transparent rounded-xl text-zinc-400 font-bold text-sm transition-all">
            เข้าร่วม Discord
          </div>
          {discordLink === '#' && (
            <div className="absolute top-4 right-4 bg-[#161a26] border border-[#1f293d] text-zinc-500 text-[10px] font-bold px-2.5 py-1 uppercase tracking-wide rounded-md">
              เร็วๆ นี้
            </div>
          )}
        </a>
        ...
        <a 
          ...
          rel="noopener noreferrer"
          className="bg-[#11131a] border border-[#1f293d] hover:border-[#1877F2] hover:shadow-[0_4px_20px_-8px_rgba(24,119,242,0.3)] transition-all p-8 flex flex-col items-center justify-center text-center group cursor-pointer relative overflow-hidden rounded-2xl shadow-sm outline-none"
          onClick={(e) => {
            if (facebookLink === '#') {
               e.preventDefault();
            }
        ...
        >
          ...
          <h2 className="text-xl font-bold text-white mb-2">Facebook Page</h2>
          <p className="text-zinc-500 text-sm font-medium">ติดตามการอัปเดตและติดต่อสอบถามผ่านทางข้อความแชทเฟสบุ๊ค</p>
          <div className="mt-6 px-6 py-2.5 bg-[#161a26] hover:bg-[#166fe5] border border-[#2d3748] group-hover:text-white group-hover:border-transparent rounded-xl text-zinc-400 font-bold text-sm transition-all">
            ติดต่อผ่าน Facebook
          </div>
          {facebookLink === '#' && (
            <div className="absolute top-4 right-4 bg-[#161a26] border border-[#1f293d] text-zinc-500 text-[10px] font-bold px-2.5 py-1 uppercase tracking-wide rounded-md">
        ...
        <a 
          ...
          rel="noopener noreferrer"
          className="bg-[#11131a] border border-[#1f293d] hover:border-[#E4405F] hover:shadow-[0_4px_20px_-8px_rgba(228,64,95,0.3)] transition-all p-8 flex flex-col items-center justify-center text-center group cursor-pointer relative overflow-hidden rounded-2xl shadow-sm outline-none"
          onClick={(e) => {
            if (instagramLink === '#') {
              e.preventDefault();
            }
        ...
        >
          ...
          <div className="mt-6 px-6 py-2.5 bg-[#161a26] hover:bg-[#d83753] border border-[#2d3748] group-hover:text-white group-hover:border-transparent rounded-xl text-zinc-400 font-bold text-sm transition-all">
            ติดตาม Instagram
          </div>
          {instagramLink === '#' && (
            <div className="absolute top-4 right-4 bg-[#161a26] border border-[#1f293d] text-zinc-500 text-[10px] font-bold px-2.5 py-1 uppercase tracking-wide rounded-md">
        ...
        <a 
          ...
          className="bg-[#11131a] border border-[#1f293d] hover:border-zinc-500 hover:shadow-[0_4px_20px_-8px_rgba(255,255,255,0.1)] transition-all p-8 flex flex-col items-center justify-center text-center group cursor-pointer relative overflow-hidden rounded-2xl shadow-sm outline-none"
          onClick={(e) => {
            if (!contactEmail) {
              e.preventDefault();
            }
        ...
        >
          ...
          <div className="mt-6 px-6 py-2.5 bg-[#161a26] hover:bg-[#2d3748] border border-[#2d3748] group-hover:text-white group-hover:border-transparent rounded-xl text-zinc-400 font-bold text-sm transition-all">
            ส่งอีเมลหาเรา
          </div>
          {!contactEmail && (
            <div className="absolute top-4 right-4 bg-[#161a26] border border-[#1f293d] text-zinc-500 text-[10px] font-bold px-2.5 py-1 uppercase tracking-wide rounded-md">
