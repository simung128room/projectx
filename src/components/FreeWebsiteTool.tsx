    ...
    <AnimatedScroll direction="up">
      <div className="font-sans px-4 pb-12 w-full max-w-5xl mx-auto mt-4 min-h-[600px]">
        ...
        <div className="mb-8 flex items-center gap-4">
          <button 
            onClick={onBack}
            className="w-10 h-10 bg-card border border-border border-2 flex items-center justify-center text-muted-foreground hover:text-white hover:bg-white/10 transition-all brut-card"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <span className="text-[10px] bg-primary text-primary-foreground text-blue-600 border border-purple-500/20 px-2.5 py-0.5 font-bold uppercase tracking-wider">
              Web Builder
            </span>
        ...
        <div className="bg-card border border-border border-2 p-6 md:p-10 relative overflow-hidden brut-card">
          {/* Background Accent */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-primary text-primary-foreground blur-[100px] pointer-events-none"></div>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              ...
              >
                <div className="mb-8 p-4 bg-primary text-primary-foreground text-blue-600 border border-purple-500/20 flex items-start gap-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                  ...
                  <div className="space-y-5">
                    <div>
                      <label className="block text-muted-foreground text-xs font-bold mb-2 ml-1 uppercase tracking-wider">ชื่อร้านค้า (Store Name)</label>
                      <input 
                        type="text" 
                        value={storeName}
                        onChange={(e) => setStoreName(e.target.value)}
                        placeholder="ตัวอย่าง: APEX Premium Shop"
                        className="w-full bg-card border border-border border-2 py-3 px-4 text-sm text-white focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all font-medium brut-card"
                      />
                    </div>
                    <div>
                      <label className="block text-muted-foreground text-xs font-bold mb-2 ml-1 uppercase tracking-wider">ลิงก์ซับโดเมนระบบ (Subdomain)</label>
                      <div className="flex border border-border border-2 bg-card overflow-hidden focus-within:border-purple-500/50 focus-within:ring-1 focus-within:ring-purple-500/50 transition-all brut-card">
                        <div className="bg-card px-3 flex items-center justify-center text-muted-foreground border-r border-border border-2 font-mono text-xs shrink-0 select-none brut-card">
                          https://
                        </div>
                        <input 
                          type="text" 
                        ...
                        />
                        <div className="bg-card px-3 flex items-center justify-center text-muted-foreground border-l border-border border-2 font-mono text-xs shrink-0 select-none brut-card">
                          .apex.xyz
                        </div>
                      </div>
                      <p className="text-muted-foreground text-[10px] mt-2 ml-1">* อักษรภาษาอังกฤษ ตัวเลข และขีดกลาง (-) เท่านั้น</p>
                    </div>
                  </div>

                  {/* Select Theme */}
                  <div>
                    <label className="block text-muted-foreground text-xs font-bold mb-3.5 ml-1 uppercase tracking-wider">เลือกสไตล์การดีไซน์ (Design Theme)</label>
                      ...
                      {THEMES.map((theme) => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-4 border cursor-pointer select-none transition-all ${isSelected ? 'border-purple-500 bg-primary text-primary-foreground' : 'border-border border-2 bg-black/30 hover:border-white/20'}`}
                          >
                            <div className="flex items-center gap-2 mb-2">
                              <span className={`w-3.5 h-3.5 ${theme.bgClass} border border-border border-2`}></span>
                              <span className="text-xs font-bold text-white">{theme.name}</span>
                            </div>
                            <div className="flex gap-1">
                              <span className="w-4 h-1.5 bg-card brut-card"></span>
                              <span className={`w-5 h-1.5 ${isSelected ? 'bg-purple-400' : 'bg-zinc-500'}`}></span>
                            </div>
                          </div>
                        );
                      })}
                ...
                <button
                  ...
                  className="w-full hover:-translate-y-0.5 bg-primary text-primary-foreground hover:bg-blue-600 disabled:opacity-40 disabled:cursor-not-allowed disabled:transform-none text-white py-4 font-bold text-sm tracking-wide transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                >
            ...
            {step === 2 && (
              ...
              >
                <div className="w-20 h-20 mb-8 relative">
                   <div className="absolute inset-0 border-4 border-purple-500/20"></div>
                   <motion.div 
                     className="absolute inset-0 border-4 border-purple-400 border-t-transparent"
                     animate={{ rotate: 360 }}
                ...
                <div className="w-full max-w-sm mt-4">
                  ...
                  <div className="w-full h-1.5 bg-card overflow-hidden brut-card">
                    <motion.div 
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="lg:col-span-5 flex flex-col justify-between py-2">
                  <div>
                    <div className="w-14 h-14 bg-primary text-primary-foreground border border-emerald-500/25 flex items-center justify-center mb-5">
                    ...
                    <div className="space-y-4 mb-8">
                      <div className="bg-card border border-border border-2 p-4 flex items-center justify-between gap-4 brut-card">
                         <div className="flex items-center gap-3 overflow-hidden">
                            ...
                            <div className="text-left overflow-hidden">
                              <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">ลิงก์ตัวอย่างสำหรับการอ้างอิง</div>
                      ...
                      <div className="bg-card border border-border border-2 p-4 flex flex-col gap-2 brut-card">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full hover:-translate-y-0.5 bg-primary text-primary-foreground hover:bg-blue-600 text-white py-3.5 font-bold text-sm tracking-wide transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                    >
                      <Download className="w-4 h-4" /> ดาวน์โหลดไฟล์เว็บ (index.html)
                    </button>
                    <button
                      onClick={() => setStep(1)}
                      className="w-full bg-card hover:bg-[#16181f] text-muted-foreground hover:text-white py-3.5 font-bold text-sm transition-all active:scale-[0.98] flex items-center justify-center gap-2 brut-card"
                    >
                      แก้ไขสไตล์ใหม่
                    </button>
                  </div>
                ...
                <div className="lg:col-span-7 flex flex-col">
                  <div className="flex items-center justify-between mb-3.5">
                    ...
                    <div className="bg-card p-1 border border-border border-2 flex gap-1 brut-card">
                      <button 
                        onClick={() => setPreviewDevice('desktop')}
                        className={`p-1.5 transition-all ${previewDevice === 'desktop' ? 'bg-purple-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                      >
                        <Monitor className="w-4 h-4" />
                      </button>
                      <button 
                        onClick={() => setPreviewDevice('mobile')}
                        className={`p-1.5 transition-all ${previewDevice === 'mobile' ? 'bg-purple-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                      >
                  ...
                  <div className={`w-full flex justify-center items-center p-4 bg-card border border-border border-2 min-h-[400px] overflow-hidden brut-card`}>
                    <motion.div 
                      layout
                      className={`relative flex flex-col transition-all duration-300 overflow-hidden ${selectedTheme.bgClass} ${previewDevice === 'mobile' ? 'w-[320px] h-[525px] border-[8px] border-white/10' : 'w-full h-[380px] border border-white/10'}`}
                    >
                      {/* Mobile Notch Indicator */}
                      {previewDevice === 'mobile' && (
                        <div className="absolute top-0 inset-x-0 h-5 bg-card flex justify-center items-center z-20 brut-card">
                          <div className="w-16 h-3.5 bg-card brut-card"></div>
                        </div>
                      )}
                      ...
                      <header className={`px-4 sm:px-6 py-4 flex items-center justify-between border-b ${selectedTheme.borderClass} ${previewDevice === 'mobile' ? 'pt-7' : ''} shrink-0`}>
                        ...
                        <span className="font-mono text-[9px] text-muted-foreground overflow-hidden text-ellipsis whitespace-nowrap max-w-[120px]">
                      ...
                      <div className="flex-1 overflow-y-auto p-4 content-scroll text-left">
                        <div className="relative mb-5 p-4 bg-gradient-to-r overflow-hidden border border-border border-2 select-none pointer-events-none">
                        ...
                        <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-3">สินค้าทั้งหมด</h4>
                        <div className={`grid ${previewDevice === 'mobile' ? 'grid-cols-2' : 'grid-cols-3'} gap-3`}>
                          {previewProducts.map((p) => (
                            <div key={p.id} className="bg-card border border-border border-2 p-2 flex flex-col justify-between h-36 brut-card">
                              <div>
                                <div className="aspect-video bg-card flex items-center justify-center text-[8px] text-muted-foreground font-bold mb-1.5 overflow-hidden brut-card">
                      ...
                      {previewDevice === 'mobile' && (
                        <div className="absolute bottom-1 inset-x-0 flex justify-center">
                          <div className="w-24 h-1 bg-card brut-card"></div>
                        </div>
                      )}
                    </motion.div>
                  </div>
