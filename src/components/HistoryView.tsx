...
export const HistoryView: React.FC<HistoryViewProps> = ({ purchaseHistory = [], topupHistory = [], usedKeysHistory = [], defaultTab = null }) => {
  ...
  const categories = [
    ...
    },
    {
      ...
      icon: ShoppingCart,
      bg: 'bg-[#3b82f6]/10 border border-blue-500/20',
      color: 'text-[#3b82f6]'
    },
    {
      id: 'key_usage',
      ...
      icon: Key,
      bg: 'bg-zinc-500/10 border border-border/20',
  ...
  const getStatusBadge = (status: string) => {
    switch(status?.toLowerCase()) {
      case 'success':
        return <span className="bg-blue-600/10 text-blue-500 px-3 py-1 text-[10px] font-medium uppercase tracking-widest border border-blue-500/20 rounded-md flex items-center gap-1.5"><div className="w-1.5 h-1.5 bg-blue-600 rounded-full"></div>สำเร็จ</span>;
      case 'pending':
      ...
      default:
        return <span className="bg-card text-muted-foreground px-3 py-1 text-[10px] font-medium uppercase tracking-widest border border-border rounded-md flex items-center gap-1.5"><div className="w-1.5 h-1.5 bg-zinc-500 rounded-full"></div>{status || 'สำเร็จ'}</span>;
    }
  };
  ...
  const getFilteredData = (categoryId: string) => {
    switch(categoryId) {
      case 'normal_product':
        return purchaseHistory.filter(p => !p.is_special).map(p => ({ ...p, type: 'normal_product', title: p.productName || 'ซื้อสินค้า', icon: ShoppingCart, color: 'text-[#3b82f6]', bg: 'bg-[#3b82f6]/10 border border-blue-500/20', money: -(p.price || 0), date: p.date || p.timestamp }));
      ...
      case 'key_usage':
        return usedKeysHistory.map(k => ({ ...k, type: 'key_usage', title: 'เปิดใช้งานคีย์', icon: Key, color: 'text-muted-foreground', bg: 'bg-purple-505/10 border border-purple-550/20', money: 0, date: k.used_at || k.date || new Date().toISOString(), productName: k.key || k.code }));
      default:
        return [];
    }
  };
  ...
  const Row = ({ index, style }: { index: number, style: React.CSSProperties }) => {
    ...
    return (
      <div style={{ ...style, paddingTop: '12px' }}>
        <div className="bg-[#0b0b0c] border border-border p-4 transition-all hover:border-[#1f2937 flex flex-col gap-4 mx-1 rounded-md shadow-sm">
          <div className="flex gap-4 items-center w-full">
            ...
            <div className="flex flex-col flex-1 min-w-0 justify-center">
              <h3 className="text-foreground font-medium text-sm sm:text-base truncate tracking-wide">{item.title}</h3>
              <div className="flex items-center justify-between mt-1">
                <span className="text-xs font-semibold text-muted-foreground">{new Date(item.date).toLocaleDateString('th-TH')}</span>
                {item.money !== 0 ? (
                  ...
                  <span className="font-medium text-xs text-muted-foreground">0 ฿</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between border-t border-border/80 pt-3">
            ...
            <button 
              ...
              className="text-[11px] sm:text-xs font-medium text-gray-700 bg-card px-4 py-2 hover:bg-[#0a0a0a] transition-all flex items-center gap-1 active:scale-95 border border-border rounded-md cursor-pointer"
            >
  ...
  return (
    <AnimatedScroll direction="up" hideOnScroll={true}>
      <div className="w-full max-w-6xl mx-auto mt-6 font-sans px-4 pb-12">
        ...
        {!currentCategory ? (
          <motion.div 
            ...
            className="space-y-4"
          >
            <div className="bg-[#070708] border border-border overflow-hidden mb-6 rounded-md shadow-md">
              <div className="p-6 md:p-8 border-b border-border bg-[#09090a]">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-blue-600/10 text-blue-500 rounded-md border border-blue-500/20">
                    <History className="w-6 h-6 " />
                  </div>
                  <div>
                    <h2 className="text-xl font-medium text-foreground leading-none mb-1.5">ประวัติสั่งซื้อ</h2>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {categories.map((cat) => (
                    <button
                      ...
                      className="group flex items-center justify-between p-4 bg-[#0a0a0b] border border-border hover:border-blue-500/30 rounded-md cursor-pointer hover:-translate-y-0.5 transition-all duration-150"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          <h3 className="text-sm font-medium text-foreground tracking-wide group-hover:text-blue-500 transition-colors">{cat.title}</h3>
                          <p className="text-[10px] sm:text-xs font-semibold text-muted-foreground uppercase tracking-tight mt-0.5">{cat.subtitle}</p>
                        </div>
                      </div>
          ...
          <motion.div 
            ...
            className="space-y-4"
          >
            <div className="bg-[#070708] border border-border overflow-hidden rounded-md shadow-md">
              <div className="p-4 sm:p-6 border-b border-border bg-[#09090a] flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button 
                    onClick={() => setCurrentCategory(null)}
                    className="p-2.5 bg-card border border-border text-muted-foreground hover:bg-[#0a0a0a] hover:text-foreground transition-all mr-2 rounded-md cursor-pointer"
                  >
                  ...
                  <div className="text-left pl-1">
                    <h2 className="text-sm sm:text-base font-medium text-foreground leading-none mb-1">{currentCategoryInfo?.title}</h2>
                    <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">{currentCategoryInfo?.subtitle}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 md:p-6 bg-transparent min-h-[400px]">
                {isLoading ? (
                  <div className="space-y-3">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="bg-[#0b0b0c] border border-border p-4 flex flex-col gap-4 rounded-md ">
                  ...
                  <div className="py-20 flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-base font-medium text-foreground mb-1">ยังไม่มีประวัติ</h3>
                    <p className="text-xs font-semibold text-muted-foreground">ยังไม่พบข้อมูลในหมวดหมู่นี้</p>
                    <button 
                      onClick={() => setCurrentCategory(null)}
                      className="mt-6 px-6 py-2.5 bg-card border border-border rounded-md text-foreground text-xs font-medium hover:bg-[#0a0a0a] transition-colors cursor-pointer"
                    >
                      ย้อนกลับ
                    </button>
                  </div>
