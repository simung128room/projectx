    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-8 select-none">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-[#0c0c0c]/90 border border-white/[0.05] rounded-xl p-4 flex flex-col justify-between">
          <span className="text-xs text-zinc-500 font-bold">Bought</span>
        ...
        <div className="bg-[#0c0c0c]/90 border border-white/[0.05] rounded-xl p-4 flex flex-col justify-between">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-zinc-600">{totalOrders}</span>
          </div>
        </div>
        <div className="bg-[#0c0c0c]/90 border border-white/[0.05] rounded-xl p-4 flex flex-col justify-between">
          <span className="text-xs text-zinc-500 font-bold"></span>
          ...
          <table className="w-full text-left text-sm text-zinc-300 min-w-[700px]">
            <thead className="bg-[#0b0b0b] border-b border-white/[0.05] text-xs uppercase font-bold text-zinc-500 tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4"></th>
                <th className="px-6 py-4 text-center w-[120px]"></th>
              </tr>
            </thead>
              ...
              {purchaseHistory.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center text-zinc-500 bg-[#0a0a0a]/50">
                    ...
                    <p className="font-bold text-zinc-400 text-base">Bought</p>
                    <p className="text-zinc-600 text-xs mt-1">Bought</p>
                  </td>
                </tr>
              ) : (
                purchaseHistory
                    ...
                    return (
                      <tr key={order.id || idx} className="hover:bg-white/[0.01] transition-colors group">
                        ...
                        <td className="px-6 py-4 max-w-[220px]">
                          <div className="font-extrabold text-white truncate text-sm">{order.productName || ''}</div>
                          {isPre && (
                            <span className="text-[10px] text-zinc-500 block mt-0.5 font-semibold">Bought</span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-right font-mono font-black text-neon-yellow text-sm">
                          {Number(order.price || order.money || 0).toLocaleString()}</td>
                          ...
                          {(status === 'success' || status === 'preorder' || status === 'completed') ? (
                            <button
                              onClick={() => setSelectedItem({
                                ...
                                secretData: order.secretData || order.key || '...',
                                type: 'normal_product'
                              })}
                ...
                return (
                  <div key={order.id || idx} className="p-5 hover:bg-white/[0.01] transition-all flex flex-col gap-4">
                    <div className="flex items-center justify-between border-b border-white/[0.03] pb-3">
                      <div>
                        <div className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest"></div>
                        <div className="text-xs font-mono font-bold text-zinc-300 mt-0.5">
                          {order.id ? order.id.substring(0, 8).toUpperCase() : `BILL-${idx + 1}`}
                        </div>
                      </div>
                      <div className="flex flex-col items-end">
                        <div className="text-[10px] text-zinc-500 font-bold">Price</div>
                        <div className="text-sm font-mono font-black text-neon-yellow mt-0.5">
                          {Number(order.price || order.money || 0).toLocaleString()}</div>
                      </div>
                    </div>

                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-wide"></span>
                      <span className="text-sm font-extrabold text-white leading-tight">{order.productName || ''}</span>
                      {isPre && <span className="text-[9px] text-zinc-500 italic mt-0.5"></span>}
                    </div>

                    <div className="flex items-center justify-between pt-1">
                      <div>
                        <span className="text-[9px] text-zinc-600 block"></span>
      ...
      <div className="mt-6 bg-[#0c0c0c] border border-[#2b2207]/40 rounded-2xl p-4 flex gap-4 items-start">
        ...
        <div className="space-y-1">
          <p className="text-xs font-bold text-zinc-200">(Pre-order Info)</p>
