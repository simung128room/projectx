      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md pointer-events-auto">
          ...
          <motion.div
            ...
            className="relative w-full max-w-[430px] bg-zinc-900 border border-zinc-800/80 shadow-2xl flex flex-col z-10 rounded-2xl overflow-hidden select-none text-foreground"
          >
            {/* Extremely Visible Close Button at the top right of dynamic pop-up */}
            <button
              onClick={handleClose}
              className="absolute top-3.5 right-3.5 z-20 w-8 h-8 rounded-full bg-red-500 hover:bg-red-600 active:scale-95 flex items-center justify-center text-foreground transition-all cursor-pointer shadow-lg shadow-red-500/25"
              aria-label="Close"
            ...
            {/* Elegant Header */}
            <div className="bg-zinc-950 text-foreground px-5 py-4 flex items-center gap-2 border-b border-zinc-800/60 shrink-0">
            ...
            <div className="p-4 bg-zinc-950 flex items-center justify-between border-t border-zinc-800/60 shrink-0 gap-4 mt-auto">
              <label className="flex items-center gap-2.5 cursor-pointer group">
                ...
                <span className="text-muted-foreground text-xs font-bold select-none group-hover:text-zinc-350 transition-colors">ไม่แสดงอีกภายใน 24 ชม.</span>
              </label>

              <button 
                onClick={handleClose} 
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-foreground text-xs font-bold uppercase tracking-wider transition-all cursor-pointer rounded-xl shrink-0 active:scale-95 shadow-md shadow-blue-500/15"
              >
                ตกลง / รับทราบ
              </button>
            </div>
