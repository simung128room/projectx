      ...
      >
        ...
        <div className="bg-white border border-zinc-200 rounded-3xl overflow-hidden shadow-sm">
          <div className="grid grid-cols-1 lg:grid-cols-12 relative z-10">
            ...
            <div className="lg:col-span-12 xl:col-span-5 p-8 md:p-12 border-b lg:border-b-0 lg:border-r border-zinc-200 bg-slate-50 relative overflow-hidden">
              ...
              <div className="mb-10 relative z-10">
                <div className="w-16 h-16 bg-blue-500 text-white flex items-center justify-center mb-6 rounded-2xl shadow-md shadow-blue-500/10">
              ...
              <div className="pt-6 border-t border-zinc-200 relative z-10">
                ...
                <button 
                  onClick={onGoToStore}
                  className="w-full flex items-center justify-between px-5 py-4 bg-white hover:bg-slate-50 text-zinc-800 hover:text-blue-600 border border-zinc-200 rounded-2xl shadow-sm transition-all active:scale-[0.98] group cursor-pointer"
                >
                  <span className="text-xs font-bold flex items-center gap-2">
                    สั่งซื้อผ่านเว็บไซต์เลย
                  </span>
                  <ChevronRight className="w-4 h-4 text-zinc-400 group-hover:text-blue-500 group-hover:translate-x-1 transition-all" />
                </button>
              </div>
            </div>
            ...
            <div className="lg:col-span-12 xl:col-span-7 p-8 md:p-12 lg:p-16 flex flex-col justify-center bg-white relative">
              <div className="max-w-md mx-auto w-full relative z-10">
                <div className="mb-10">
                   <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-zinc-200 text-[10px] font-bold uppercase tracking-widest text-zinc-500 rounded-lg mb-4">
                     <KeyIcon className="w-3 h-3" /> Activation Center
                   </div>
                   <h3 className="text-2xl font-bold text-zinc-900 mb-2">Redeem Code</h3>
                   <p className="text-zinc-500 text-sm font-medium">วางคีย์ 16 หลักของคุณที่นี่เพื่อยืนยันสิทธิ์พรีเมียม</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <div className={`relative transition-all duration-300 bg-white border ${isFocused ? 'border-blue-500 shadow-md shadow-blue-500/5' : 'border-zinc-200 hover:border-zinc-300'} rounded-2xl overflow-hidden ${!isLoggedIn ? 'opacity-50 select-none pointer-events-none' : ''}`}>
                      <div className="flex items-center gap-4 px-5">
                        <KeyIcon className={`w-6 h-6 transition-colors duration-300 ${isFocused || keyInput ? 'text-blue-550' : 'text-zinc-400'}`} />
                        <input 
                          required
                          disabled={!isLoggedIn}
                          value={keyInput}
                    ...
                    {!isLoggedIn && (
                      <div className="mt-4 text-center bg-slate-50 border border-zinc-200/80 p-5 rounded-2xl">
                        <p className="text-xs font-bold text-zinc-500 mb-3 flex items-center justify-center gap-1.5">
                          <AlertCircle className="w-4 h-4 text-zinc-400" /> กรุณาเข้าสู่ระบบก่อนกรอกคีย์
                        </p>
                        <button type="button" onClick={onLoginClick} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm cursor-pointer whitespace-nowrap">เข้าสู่ระบบ / สมัครสมาชิก</button>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    ...
                    <div className="relative h-full w-full bg-blue-600 text-white hover:bg-blue-700 rounded-2xl font-bold text-sm uppercase tracking-widest flex items-center justify-center gap-3 active:scale-[0.98] transition-all shadow-md">
                      เปิดใช้งานเดี๋ยวนี้
