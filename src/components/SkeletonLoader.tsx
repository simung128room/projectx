...
export const SkeletonHomeLoader: React.FC = () => {
  return (
    <div className="w-full flex flex-col gap-6" id="skeleton-home-loader">
      ...
      <div className="relative w-full rounded-md bg-white/[0.01] border border-[#1e1e1e] p-6 overflow-hidden min-h-[220px] sm:min-h-[300px] flex flex-col justify-end shimmer-bg">
        {/* Decorative elements */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4" id="skeleton-stats-row">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="relative bg-[#0d0d0d] border border-[#1e1e1e] rounded-md p-5 overflow-hidden flex flex-col gap-2 shimmer-bg">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" id="skeleton-shortcuts-row">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-[#0d0d0d] border border-[#1e1e1e] rounded-md p-4 flex items-center gap-3.5 overflow-hidden relative shimmer-bg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4" id="skeleton-categories-row">
        {[...Array(2)].map((_, i) => (
          <div key={i} className="border border-[#1e1e1e] bg-[#0c0c0e] rounded-md overflow-hidden shadow-sm flex flex-col relative h-[160px] shimmer-bg">
            <div className="w-full h-16 bg-white/[0.04] border-b border-[#1e1e1e] " />
            <div className="p-4 flex flex-col justify-between flex-1">
              <div className="w-1/3 h-5 rounded bg-white/[0.06] " />
              <div className="flex items-center justify-between mt-4">
                <div className="w-24 h-4 rounded bg-white/[0.06] " />
                <div className="w-16 h-6 rounded-md bg-[#141416] border border-[#1e1e1e] " />
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4" id="skeleton-products-row">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="bg-[#0c0c0e] border border-[#1e1e1e] rounded-md p-3 flex flex-col relative h-[280px] overflow-hidden shimmer-bg">
            ...
            <div className="flex justify-between items-center mt-auto pt-2 border-t border-[#1e1e1e]">
...
export const SkeletonGenericLoader: React.FC = () => {
  return (
    <div className="w-full flex flex-col gap-6" id="skeleton-generic-loader">
      ...
      <div className="bg-white/[0.01] border border-[#1e1e1e] rounded-md p-6 flex flex-col gap-4 relative overflow-hidden shimmer-bg">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="flex flex-col gap-2 py-3 border-b border-[#1e1e1e] last:border-0 shadow-sm">
