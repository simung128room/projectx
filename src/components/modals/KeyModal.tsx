  ...
  return (
    <div className="fixed inset-0 bg-card flex items-center justify-center p-4 z-[70] font-sans animate-in zoom-in-95 duration-200 overflow-y-auto ">
      <div className="bg-card border-[#374151]  border border-[#364153]/25 p-6 sm:p-8 max-w-lg w-full relative overflow-hidden my-8 ">
          ...
          {vipTab === 'key' && (
              ...
              <div>
                  ...
                  <div className="relative">
                    ...
                    <input required name="key" type="text" className="w-full bg-card border border-[#374151]  py-3 pl-10 pr-4 text-sm focus:border-[#364153] outline-none text-foreground transition-all placeholder:text-muted-foreground " placeholder="APEXSTORE-XXXXX-XXXXX" />
                  </div>
              </div>
              <button type="submit" className="w-full from-[#364153] to-emerald-700 hover:from-emerald-400 hover:to-emerald-600 text-foreground font-medium py-3.5 transition-all mt-4">
                เปิดใช้งาน
              </button>
            </form>
          )}
        ...
        <button onClick={onClose} className="absolute top-6 right-6 text-muted-foreground hover:text-foreground transition-colors bg-card hover:bg-[#1e1e1e] p-2 z-20 ">
          <X className="w-4 h-4" />
        </button>
      </div>
    </div>
